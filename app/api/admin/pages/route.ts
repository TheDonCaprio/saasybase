...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await listSitePagesPaginated({
      page,
      limit,
      status,
      search,
    ...
    return NextResponse.json({
      pages: result.pages.map(toSitePageDTO),
  ...
  trashedCount: result.overallTotals?.trashed,
    ...
    Logger.error('Failed to list site pages', {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const guard = toAuthGuardErrorResponse(error);
    if (guard) return guard;
    Logger.error('Failed to create site page', {
      error: error instanceof Error ? error.message : String(error)
    });
