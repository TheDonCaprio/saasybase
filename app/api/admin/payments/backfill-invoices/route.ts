...
async function fetchPaymentIntentFromSubscription(stripeSubscriptionId: string) {
...
export async function POST(req: NextRequest) {
  ...
  const rl = await adminRateLimit(userId, req, 'admin-payments:backfill', { limit: 60, windowMs: 120_000 });
    ...
    // Get payments that are missing stripePaymentIntentId
    // Note: `stripeInvoiceId` does not exist on the Payment model in Prisma schema,
    // so only query payments that have a checkout session ID to avoid Prisma validation errors.
  ...
  for (const payment of payments) {
      try {
        ...
        Logger.info(`Processing payment ${payment.id}`, {
          ...
          hasSubscription: !!payment.subscription?.stripeSubscriptionId,
          checkoutSessionId: payment.stripeCheckoutSessionId,
          subscriptionId: payment.subscription?.stripeSubscriptionId,
        });

        // Try to get payment intent ID from checkout session first
        if (payment.stripeCheckoutSessionId && !paymentIntentId) {
          try {
            Logger.info(`Retrieving checkout session`, { sessionId: payment.stripeCheckoutSessionId });
            const sessionRaw = await stripe.checkout.sessions.retrieve(String(payment.stripeCheckoutSessionId));
            ...
            Logger.warn('Error retrieving checkout session during backfill', { sessionId: payment.stripeCheckoutSessionId, error: toError(e) });
          }
        }

        if (!paymentIntentId && payment.subscription?.stripeSubscriptionId) {
          const subResult = await fetchPaymentIntentFromSubscription(payment.subscription.stripeSubscriptionId);
          if (subResult) {
        ...
        if (paymentIntentId) {
          ...
          const existing = await prisma.payment.findUnique({ where: { stripePaymentIntentId: paymentIntentId } });
          if (existing && existing.id !== payment.id) {
            ...
            await prisma.payment.update({ where: { id: payment.id }, data: { stripePaymentIntentId: paymentIntentId } });
    ...
    return NextResponse.json(
      { error: 'Failed to backfill payment intent IDs', details: e.message },
      { status: 500 }
    );
  }
}
