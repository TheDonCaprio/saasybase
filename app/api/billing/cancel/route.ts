...
export async function POST(req: Request) {
  ...
  const subscription = await prisma.subscription.findFirst({ where: { userId, status: 'ACTIVE', expiresAt: { gt: new Date() } }, include: { plan: true } });
  ...
  try {
    ...
    Logger.info('User scheduled subscription cancellation', { userId, subscriptionId: subscription.id, externalSubscriptionId: subId, ip });
