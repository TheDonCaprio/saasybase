...
function StatPill({ label, value, tone }: { label: string; value: number; tone?: 'neutral' | 'warn' | 'danger' }) {
  const cls =
    tone === 'danger'
      ? 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-500/10 dark:text-rose-200 dark:border-rose-500/20'
      : tone === 'warn'
        ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-200 dark:border-amber-500/20'
        : 'bg-white text-slate-700 border-slate-200 dark:bg-neutral-800 dark:text-neutral-200 dark:border-neutral-700';
...
export function MaintenanceTools() {
  ...
  return (
    <section className="space-y-4">
      <div className={dashboardPanelClass('space-y-5 p-5')}>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold text-slate-900 dark:text-neutral-100">Discounted subscription price cache</h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-60 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-100 dark:hover:bg-neutral-700"
            >
            ...
            <button
              ...
              disabled={loading || running}
              className="inline-flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-800 transition hover:bg-amber-100 disabled:opacity-60 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100 dark:hover:bg-amber-500/15"
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm text-rose-800 transition hover:bg-rose-100 disabled:opacity-60 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-100 dark:hover:bg-rose-500/15"
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-neutral-400">Stale pending threshold (minutes)</label>
            <input
              type="number"
              min={1}
              max={1440}
              ...
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-60 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100"
            />
            <p className="text-xs text-slate-500 dark:text-neutral-500">Pending keys are just locks. If a request crashed, this clears the stuck lock.</p>
          </div>

          <div className="space-y-2">
            <label className="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-neutral-400">Old ready threshold (days)</label>
            <input
              type="number"
              min={1}
              max={3650}
              ...
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-60 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100"
            />
            <p className="text-xs text-slate-500 dark:text-neutral-500">Ready keys enable reuse. Deleting old ones reduces DB clutter but may recreate provider plans later.</p>
          </div>
        </div>
        ...
        {lastResult ? (
          <div className={dashboardMutedPanelClass('mt-4 p-3 text-sm text-slate-700 dark:text-neutral-200')}>
            <div className="flex flex-wrap items-center gap-2">
        ...
        {stats?.prefix ? (
          <p className="mt-3 text-xs text-slate-500 dark:text-neutral-500">Key prefix: {stats.prefix}</p>
        ) : null}
      </div>
    </section>
  );
