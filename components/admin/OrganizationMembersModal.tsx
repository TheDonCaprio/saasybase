...
export function OrganizationMembersModal({ orgId, orgName, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60000] flex items-start justify-center overflow-y-auto px-4 py-6 sm:py-10">
      ...
      />
      <div className="relative z-[60001] mx-4 flex w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl sm:mx-auto dark:border-neutral-800/80 dark:bg-neutral-950/95">
            ...
            <>
              <section>
                ...
                <div className="space-y-3 md:hidden">
                  {members.map((member) => (
                    <div key={member.id} className="rounded-lg border border-slate-200 bg-slate-50/50 p-3 dark:border-neutral-800 dark:bg-neutral-900/30 sm:p-4">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-200 pt-2.5 text-xs text-slate-500 dark:border-neutral-800 dark:text-neutral-400">
                        <div>
                      ...
                      <div className="mt-3 border-t border-slate-200 pt-2.5 dark:border-neutral-800">
                        <button
                          type="button"
                          onClick={() => setMemberToRemove(member)}
                          disabled={removingMemberId === member.id}
                          className="inline-flex items-center gap-2 rounded-md bg-red-600 px-2.5 py-1 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-red-500 dark:hover:bg-red-600"
                        >
                ...
                <div className="hidden md:block overflow-hidden rounded-lg border border-slate-200 dark:border-neutral-800">
                  <table className="min-w-full divide-y divide-slate-200 dark:divide-neutral-800 text-sm">
                    <thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wide text-slate-500 dark:bg-neutral-900/60 dark:text-neutral-400">
                      <tr>
                        <th className="px-3 py-2.5">Member</th>
                        <th className="px-3 py-2.5">Role</th>
                        <th className="px-3 py-2.5">{formatTokenPoolStrategyLabel(tokenPoolStrategy)}</th>
                        <th className="px-3 py-2.5">Cap override</th>
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-neutral-800">
                      {members.map((member) => (
                        <tr key={member.id} className="bg-white dark:bg-neutral-900/20">
                          ...
                          <td className="px-3 py-2.5 text-slate-600 dark:text-neutral-300">{member.role}</td>
                          <td className="px-3 py-2.5">
                          ...
                          <td className="px-3 py-2.5 text-slate-600 dark:text-neutral-300">{member.memberTokenCapOverride ?? 'None'}</td>
                          <td className="px-3 py-2.5">
                            <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-600 dark:bg-neutral-800 dark:text-neutral-200">{member.status}</span>
                          </td>
              ...
              <section>
                ...
                <div className="space-y-3 md:hidden">
                  {invites.map((invite) => (
                    <div key={invite.id} className="rounded-lg border border-slate-200 bg-slate-50/50 p-3 dark:border-neutral-800 dark:bg-neutral-900/30 sm:p-4">
                      <div className="mb-2 flex items-start justify-between">
                    ...
                    <tbody className="divide-y divide-slate-200 dark:divide-neutral-800">
                      {invites.map((invite) => (
                        <tr key={invite.id} className="bg-white dark:bg-neutral-900/20">
                          <td className="px-3 py-2.5 text-slate-900 dark:text-white">{invite.email}</td>
                          <td className="px-3 py-2.5 text-slate-600 dark:text-neutral-300">{invite.role}</td>
                          <td className="px-3 py-2.5 text-slate-600 dark:text-neutral-300">{invite.status}</td>
                          <td className="px-3 py-2.5 text-slate-500 dark:text-neutral-400">{invite.expiresAt ? new Date(invite.expiresAt).toLocaleDateString() : '—'}</td>
                        </tr>
                      ))}
                      {invites.length === 0 && (
                        <tr>
                          <td className="px-3 py-3 text-center text-slate-500 dark:text-neutral-500" colSpan={4}>No pending invites</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
        ...
        <div className="flex justify-end gap-2.5 border-t border-slate-200 px-5 py-4 dark:border-neutral-800">
          <button onClick={onClose} className="rounded-md border border-slate-200 px-3.5 py-2 text-sm text-slate-600 hover:bg-slate-50 dark:border-neutral-800 dark:text-neutral-300 dark:hover:bg-neutral-800">Close</button>
        </div>
