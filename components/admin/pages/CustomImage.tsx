...
function ResizableImageComponent(props: NodeViewProps) {
  ...
  // Use capture phase to ensure we get the events before the editor
    ...
    return () => {
      console.log('Cleaning up resize listeners');
  ...
  // Note: node.attrs.style exists but is not used directly here because
  // the NodeView applies styles via inline setProperty when necessary.
  ...
  return (
    <NodeViewWrapper
      ...
      data-selected={showHandles ? 'true' : 'false'}
      data-resizing={isResizing ? 'true' : 'false'}
      as="div"
      contentEditable={false}
      style={isResizing ? { userSelect: 'none', pointerEvents: 'auto' } : undefined}
    >
...
export const CustomImage = TiptapImage.extend({
  addAttributes() {
    return {
      ...
      'data-external': {
        default: null,
        parseHTML: (element: HTMLElement) => element.getAttribute('data-external'),
        renderHTML: (attributes: ImageAttributes) => {
          if (!attributes['data-external']) return {};
          return { 'data-external': attributes['data-external'] };
        },
      },
    };
  },
