...
function FooterShell() {
  return <div className="w-full border-t border-neutral-200 pb-2 pt-4 dark:border-neutral-800/60" aria-hidden="true" />;
}
...
export function SidebarFooter() {
  ...
  return (
    <ClientOnly fallback={<FooterShell />}>
      <div className="w-full space-y-3 border-t border-neutral-200 pb-2 pt-4 dark:border-neutral-800/60" ref={menuRef}>
        ...
        <div className={`relative px-2 ${isMenuOpen ? 'z-[80]' : 'z-[55]'}`}>
          {isMenuOpen && (
            <div className="absolute bottom-full left-0 z-[85] mb-2 w-full min-w-[200px] overflow-hidden rounded-xl border border-neutral-200 bg-white p-1 shadow-xl shadow-black/5 ring-1 ring-black/5 dark:border-neutral-800 dark:bg-neutral-950 dark:shadow-black/30 dark:ring-white/10">
              <Link
                ...
                className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-neutral-700 transition-colors hover:bg-neutral-100 dark:text-neutral-300 dark:hover:bg-neutral-800"
              >
              ...
              <AuthSignOutButton>
                <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-red-600 transition-colors hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10">
          ...
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-lg px-2 py-1.5 transition-all duration-200 ${
              isMenuOpen
                ? 'bg-neutral-100 ring-1 ring-neutral-200 dark:bg-neutral-800 dark:ring-neutral-700'
                : 'hover:bg-neutral-100 dark:hover:bg-neutral-800'
            }`}
          >
            <div className="relative flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-full border border-violet-200 bg-violet-100 shadow-sm dark:border-violet-500/20 dark:bg-violet-900/40">
              {user.imageUrl ? (
            ...
            <div className="flex flex-1 flex-col items-start truncate leading-tight">
              <span className="w-full truncate text-left text-[13px] font-semibold text-neutral-900 dark:text-neutral-100">
                {user.fullName || user.primaryEmailAddress?.emailAddress.split('@')[0]}
              </span>
              <span className="w-full truncate text-left text-[10px] font-medium text-neutral-500 dark:text-neutral-400">
