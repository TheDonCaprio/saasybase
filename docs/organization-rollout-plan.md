...
> Status: implemented in the live app as of April 2026. This document is retained as the rollout record and as context for remaining policy/editorial follow-up.
...
- ✅ `.env.example`, the payment catalog sync flow, and dev seeds support representative team tiers without requiring manual `SUBSCRIPTION_PRICE_*` environment variables in the standard setup.
...
2. Finalize seat-limit and member-cap policy decisions now that the core organization flow is live.
3. Add or expand regression tests around the cleanup hooks triggered when subscriptions expire.
