...
function getStorageBackend() {
  return process.env.FILE_STORAGE || process.env.LOGO_STORAGE || 'fs';
}

function getStorageBucket() {
  return process.env.FILE_S3_BUCKET || process.env.LOGO_S3_BUCKET;
}

function getStorageEndpoint() {
  return process.env.FILE_S3_ENDPOINT || process.env.LOGO_S3_ENDPOINT || undefined;
}

function getStorageCdnDomain() {
  return process.env.FILE_CDN_DOMAIN || process.env.LOGO_CDN_DOMAIN || null;
}
...
async function listAdminFilesFromS3({ limit, cursor, search, scope = FILE_SCOPE }) {
  ...
  if (!bucket || !region) {
    throw new Error('S3 not configured (FILE_S3_BUCKET or AWS_REGION missing; legacy LOGO_S3_BUCKET also supported)');
  }
