...
export async function saveAdminFile({ buffer, filename, mimetype }) {
  return saveAsset({ buffer, filename, mimetype, scope: FILE_SCOPE });
}
...
function encodeS3Cursor({ token, pending }) {
  const sanitizedPending = Array.isArray(pending)
    ? pending
  ...
  if (!normalizedToken && sanitizedPending.length === 0) {
    return null;
  }
...
function decodeS3Cursor(cursor) {
  if (!cursor) {
  ...
  try {
    ...
    // Treat as legacy raw continuation token
    return { token: String(cursor), pending: [] };
  }

  return { token: null, pending: [] };
}

export async function listAdminFiles({ limit = 20, cursor = null, search = null } = {}) {
  ...
  if (storage === 's3') {
    return listAdminFilesFromS3({ limit: safeLimit, cursor, search: normalizedSearch });
  }

  return listAdminFilesFromFs({ limit: safeLimit, cursor, search: normalizedSearch });
}
...
export async function deleteAdminFile({ key }) {
  ...
  if (!normalizedKey.startsWith('files/')) {
...
async function listAdminFilesFromS3({ limit, cursor, search }) {
  ...
  const { token: initialToken, pending: initialPending } = decodeS3Cursor(cursor);
  const matches = [];
  const overflow = [];
  ...
  const emitMatch = (file) => {
    if (matches.length < limit) {
      matches.push(file);
    } else {
      overflow.push({ key: file.key, size: file.size, uploadedAt: file.uploadedAt });
    }
  };

  for (const pendingItem of initialPending) {
    emitMatch(buildInfo(pendingItem.key, pendingItem.size, pendingItem.uploadedAt));
    if (matches.length >= limit) {
      // If we already filled the page from pending items, we can stop here.
      // Remaining pending entries will be re-encoded into the cursor below.
    }
  }
  ...
  while (matches.length < limit && (scans === 0 || hasMoreResponses) && scans < MAX_SCANS) {
    ...
    hasMoreResponses = response.IsTruncated === true && Boolean(continuationToken);
  ...
  const nextCursor = encodeS3Cursor({
...
async function listAdminFilesFromFs({ limit, cursor, search }) {
...
async function deleteAdminFileFromFs(key) {
  ...
  while (directory.startsWith(filesRoot) && directory.length >= filesRoot.length) {
