...
export async function deleteAdminFile({ key, scope = FILE_SCOPE }) {
...
async function listAdminFilesFromS3({ limit, cursor, search, scope = FILE_SCOPE }) {
  ...
  else if (scope === BLOG_SCOPE) prefixToUse = 'blog/';
  ...
  // Sort newest-first to match the local filesystem behaviour (mtimeMs desc).
  matches.sort((a, b) => {
...
async function listAdminFilesFromFs({ limit, cursor, search, scope = FILE_SCOPE }) {
  ...
  else if (scope === BLOG_SCOPE) subdir = 'blog';
  ...
  const filesDirRel = path.relative(uploadsDir, filesDir).split(path.sep).join('/');
