...
export function normalizeRobotsTxtContent(raw: string | null | undefined): string {
  return typeof raw === 'string'
    ? raw.replace(/\r\n?/g, '\n').trim()
    : '';
}

export function buildRobotsTxtContent(options: {
  siteUrl: string;
  sitemapUrl: string;
  noIndexSite: boolean;
  ...
  if (options.noIndexSite) {
    lines.push('# Sitewide no-index is enabled. Crawlers are asked not to index this site.');
  }

  lines.push('User-agent: *');
  lines.push(options.noIndexSite ? 'Disallow: /' : 'Allow: /');
  lines.push(`Sitemap: ${options.sitemapUrl}`);
  lines.push(`Host: ${options.siteUrl}`);

  const customContent = normalizeRobotsTxtContent(options.customContent);
  if (customContent) {
    lines.push('');
    lines.push('# Custom robots.txt directives');
